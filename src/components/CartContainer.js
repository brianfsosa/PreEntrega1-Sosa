...
function CartContainer() {
  ...
  return (
    <div className="h-screen text-white ml-8 text-center  p-8">
      {cart.map((prod) => {
        return(
          <div className="flex m-8 gap-32 ">
            <img className="w-12 h-12 rounded-md border border-gray-300 hover:border-gray-500 transition duration-300 ease-in-out" src={prod.img} alt={prod.nombre}></img>
      ...
      (
      <div>
      <p className="text-6xl m-2 ">${getTotalPriceInCart()}<i className="p-2 text-2xl fa-solid fa-coins text-[#FFD700]"></i></p> 
      <Link to={'/checkout'}>
        <button
